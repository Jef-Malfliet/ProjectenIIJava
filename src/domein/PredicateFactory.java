...
//    }
    ...
    private static Predicate genPredActiviteit(List<String> parameters) {
        ...
        if (!isNull(parameters)) {
            ...
            String aDatum = parameters.get(2);
            ...
            Predicate<Activiteit> onDatum = a -> String.format("%d", a.getBeginDatum().getYear()).equals(aDatum);
            ...
            if (!isNullOrEmpty(aDatum)) {
                if (!aDatum.equalsIgnoreCase("alles")) {
                    full = full.and(onDatum);
                }  
            }
        }
        return full;
